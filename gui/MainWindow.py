...
class MainWindow:
    ...
    def __init__(self, parser, repository, painter, cycles):
        self.filename = None
    ...
    def create_buttons(self):
        self.button_select_file = Button(self.window, text="Seleccionar fichero...", font=("Arial", 12),
                                         command=self.parse_file)
        self.button_select_file.pack(side=TOP, fill=BOTH, padx=100, pady=10)
        self.button_add_to_bd = Button(self.window, text="Añadir a base de datos", font=("Arial", 12),
                                       command=self.insert_data)
        self.button_add_to_bd.pack(side=TOP, fill=BOTH, padx=100, pady=10)
        self.button_show_data = Button(self.window, text="Mostrar gráficos...", font=("Arial", 12),
                                       command=self.open_graphic_dialog)
    ...
    def show_info(self):
        messagebox.showinfo("Acerca de",
                            "License: GNU General Public License v3.0\n\nAuthor: Diego González Suárez - Universidad "
                            "de Oviedo.\n\nContact Me: uo276406@uniovi.es - diegogs1451@outlook.es\n\nSource Code: "
                            "https://github.com/uo276406/P48Manager")
    ...
    def parse_file(self):
        self.filename = filedialog.askopenfilename(initialdir="C:/ESIOS/p48cierre", title="Seleccione un fichero",
    ...
    def insert_data(self):
        try:
            ...
            messagebox.showinfo("Operación inserción",
                                "Datos añadidos correctamente: " + str(number) + " filas añadidas")
        except Exception as e:
            messagebox.showerror("Operación de inserción", e.__str__())

    def open_graphic_dialog(self):
        GraphicDialog(self.window, self.painter, self.cycles, self.repository)
